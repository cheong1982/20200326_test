...
class ReusableStrategy(BaseStrategy):
    ...
    class ConnectionPool(object):
        ...
        def __init__(self, connection):
            if not hasattr(self, 'workers'):
                ...
                self.main_connection = connection
                self.workers = []
        ...
        def __str__(self):
            ...
            s += 'MASTER CONN: ' + str(self.main_connection) + linesep
            s += 'WORKERS:'
            if self.workers:
        ...
        def rebind_pool(self):
            for worker in self.workers:
                with worker.lock:
                    worker.connection.rebind(self.main_connection.user,
                                             self.main_connection.password,
                                             self.main_connection.authentication,
                                             self.main_connection.sasl_mechanism,
                                             self.main_connection.sasl_credentials)
        ...
        def create_pool(self):
            ...
            self.workers = [ReusableStrategy.PooledConnectionWorker(self.main_connection, self.request_queue) for _ in range(self.pool_size)]
    ...
    class PooledConnectionThread(Thread):
        ...
        """
        def __init__(self, worker, main_connection):
        ...
        def run(self):
            ...
            terminate = False
            pool = self.main_connection.strategy.pool
            while not terminate:
                ...
                with self.worker.lock:
                    ...
                    if counter == TERMINATE_REUSABLE:
                        ...
                        if (datetime.now() - self.worker.creation_time).seconds >= self.main_connection.strategy.pool.lifetime:  # destroy and create a new connection
                            try:
                                self.worker.connection.unbind()
                            except LDAPExceptionError:
                                pass
    ...
    class PooledConnectionWorker(object):
        ...
        def __str__(self):
            ...
            s += ' - time to live: ' + str(self.main_connection.strategy.pool.lifetime - (datetime.now() - self.creation_time).seconds)
        ...
        def new_connection(self):
            ...
            # noinspection PyProtectedMember
            self.connection = Connection(server=self.main_connection.server_pool if self.main_connection.server_pool else self.main_connection.server,
                                         user=self.main_connection.user,
                                         password=self.main_connection.password,
                                         auto_bind=AUTO_BIND_NONE,  # do not perform auto_bind because it reads again the schema
                                         version=self.main_connection.version,
                                         authentication=self.main_connection.authentication,
                                         client_strategy=RESTARTABLE,
                                         auto_referrals=self.main_connection.auto_referrals,
                                         auto_range=self.main_connection.auto_range,
                                         sasl_mechanism=self.main_connection.sasl_mechanism,
                                         sasl_credentials=self.main_connection.sasl_credentials,
                                         check_names=self.main_connection.check_names,
                                         collect_usage=self.main_connection._usage,
                                         read_only=self.main_connection.read_only,
                                         raise_exceptions=self.main_connection.raise_exceptions,
                                         lazy=False,
                                         fast_decoder=self.main_connection.fast_decoder,
                                         receive_timeout=self.main_connection.receive_timeout,
                                         return_empty_attributes=self.main_connection.empty_attributes)

            # simulates auto_bind, always with read_server_info=False
            if self.main_connection.auto_bind and self.main_connection.auto_bind != AUTO_BIND_NONE:
                if log_enabled(BASIC):
                    log(BASIC, 'performing automatic bind for <%s>', self.connection)
                self.connection.open(read_server_info=False)
                if self.main_connection.auto_bind == AUTO_BIND_NO_TLS:
                    self.connection.bind(read_server_info=False)
                elif self.main_connection.auto_bind == AUTO_BIND_TLS_BEFORE_BIND:
                    self.connection.start_tls(read_server_info=False)
                    self.connection.bind(read_server_info=False)
                elif self.main_connection.auto_bind == AUTO_BIND_TLS_AFTER_BIND:
                    self.connection.bind(read_server_info=False)
                    self.connection.start_tls(read_server_info=False)

            if self.main_connection.server_pool:
                self.connection.server_pool = self.main_connection.server_pool
